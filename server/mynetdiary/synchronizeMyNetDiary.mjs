...
function throwIfAborted(signal) {
  if (signal?.aborted) throw new Error('MyNetDiary sync was interrupted.')
}

export async function synchronizeMyNetDiary({ ownHandle, signal } = {}) {
  let headlessBrowser
  ...
  try {
    ...
    releaseHandle = ownHandle?.({ close: () => headlessBrowser?.close().catch(() => undefined) })
    throwIfAborted(signal)
