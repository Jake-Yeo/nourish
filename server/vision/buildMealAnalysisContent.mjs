...
3. Compare the food footprint and pile height with those references across multiple angles while accounting for perspective. Use ranges for the possible dimensions of non-standard bowls, platters, utensils, and hands rather than silently assuming one exact size.
4. For a mixed dish, estimate the total occupied area/volume, then estimate its meat-to-vegetable ratio. Distinguish meat from onions, sauce, and other vegetables instead of treating the entire pile as meat or assigning a generic meat portion.
5. Convert the visually supported volume into low, central, and high cooked-weight estimates using a plausible food-density range. If scale evidence is weak, widen the range; do not collapse it to a conventional 5–6 oz restaurant prior.
6. Cross-check the result against another visible vessel or object and against before/after photos when available. Use the visually supported central estimate unless the evidence is clearly skewed toward one end.

In the assumptions, concisely record the scale references, footprint/height or vessel comparison, meat share, and low/central/high portion range that actually drove the estimate. Merely saying that a bowl or chopsticks gave “rough context” is not enough; explain what comparison they supported. If no reliable scale reference is visible, state that plainly and keep a wide uncertainty range.
