...
export function runBounded(command, args, signal, onOwnedProcess) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => fail('OpenClaw meal analysis timed out.', 504), 300_000)
...
export function buildOpenClawArguments(prompt, imagePath, sessionId = randomUUID()) {
  ...
  return ['agent', '--agent', 'main', '--session-key', `agent:main:nourish-analysis-${sessionId}`, '--model', OPENCLAW_MODEL, '--thinking', 'medium', '--timeout', '300', '--json', '--message', `${prompt}\n\n${imageInstruction}`]
}
export async function requestMealEstimate(item, overallNote, signal, onOwnedProcess) {
  ...
  try {
    ...
    const output = await runBounded(openClaw, buildOpenClawArguments(prompt, artifact.imagePath), signal, onOwnedProcess)
    return parseMealEstimateResponse(parseOpenClawAgentResponse(output))
