...
export function isMealEstimate(value) {
  const rootKeys = ['mealName', 'confidence', 'summary', 'assumptions', 'researchDisclosure', 'items', 'totals']
  ...
  return exactKeys(item, itemKeys) && validText(item.name) && validText(item.description)
    && validText(item.portion) && validNutrients(item.nutrients) && validNutrients(value.totals)
}
...
function sanitizeEstimate(estimate) {
  ...
  const required = [sanitized.mealName, sanitized.summary, sanitized.researchDisclosure.summary, ...sanitized.assumptions]
