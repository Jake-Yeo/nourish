...
export function initializeAnalysisJobSchema(database) {
  ...
  if (!columns.has('diary_update_pending')) database.exec('ALTER TABLE meal_analysis_jobs ADD COLUMN diary_update_pending INTEGER NOT NULL DEFAULT 0')
  if (!columns.has('previous_diary_update_pending')) database.exec('ALTER TABLE meal_analysis_jobs ADD COLUMN previous_diary_update_pending INTEGER')
}

const selectColumns = 'id,status,request_json,result_json,error,created_at,updated_at,started_at,finished_at,attempt,logged_at,logged_meal_id,diary_update_pending'
const mapJob = row => row && ({ id: row.id, status: row.status, source: JSON.parse(row.request_json), result: row.result_json ? JSON.parse(row.result_json) : null, error: row.error, createdAt: row.created_at, updatedAt: row.updated_at, startedAt: row.started_at, finishedAt: row.finished_at, attempt: row.attempt, loggedAt: row.logged_at, loggedMealId: row.logged_meal_id, diaryUpdatePending: Boolean(row.diary_update_pending) })
...
export function createAnalysisJob(database, request, idempotencyKey) {
  ...
  return runImmediateTransaction(database, () => {
    ...
    const job = { id: randomUUID(), status: 'queued', source: request, result: null, error: null, createdAt: now, updatedAt: now, startedAt: null, finishedAt: null, attempt: 1, loggedAt: null, loggedMealId: null, diaryUpdatePending: false }
...
export function rerunAnalysisJob(database, id, request) {
  return runImmediateTransaction(database, () => {
    const current = getAnalysisJob(database, id)
    if (!current || current.status !== 'completed') return null
    if (current.loggedAt && (request.date !== current.source.date || request.mealType !== current.source.mealType || request.items.length !== current.source.items.length || request.items.some((item, index) => item.id !== current.source.items[index].id))) throw Object.assign(new Error('Logged analysis re-runs must preserve the linked date, meal, and item set.'), { code: 'LOGGED_RERUN_SCOPE' })
    const now = new Date().toISOString()
    const update = database.prepare("UPDATE meal_analysis_jobs SET previous_request_json=request_json,previous_result_json=result_json,previous_diary_update_pending=diary_update_pending,request_json=?,status='queued',result_json=NULL,diary_update_pending=CASE WHEN logged_at IS NULL THEN 0 ELSE 1 END,error=NULL,updated_at=?,started_at=NULL,finished_at=NULL,attempt=attempt+1 WHERE id=? AND status='completed'").run(JSON.stringify(request), now, id)
    return update.changes === 1 ? getAnalysisJob(database, id) : null
  })
}
...
export function linkAnalysisJobToMeal(database, id, mealId) { const now = new Date().toISOString(); return database.prepare("UPDATE meal_analysis_jobs SET logged_at=?,logged_meal_id=?,diary_update_pending=0,updated_at=? WHERE id=? AND status='completed' AND logged_at IS NULL").run(now, mealId, now, id).changes === 1 }
export function markAnalysisDiaryUpdated(database, id, mealId) { const now = new Date().toISOString(); return database.prepare("UPDATE meal_analysis_jobs SET diary_update_pending=0,updated_at=? WHERE id=? AND logged_meal_id=? AND status='completed' AND logged_at IS NOT NULL AND diary_update_pending=1").run(now, id, mealId).changes === 1 }
