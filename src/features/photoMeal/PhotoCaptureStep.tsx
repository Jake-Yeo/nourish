...
export function PhotoCaptureStep(props: Props) {
  ...
  return <div>
    ...
    <Button fullWidth disabled={!props.items.length} onClick={props.onAnalyze}><Sparkles />Analyze {props.items.length || ''} item{props.items.length === 1 ? '' : 's'}</Button>
  </div>
}
