...
export function PhotoCaptureStep(props: Props) {
  ...
  return <fieldset disabled={props.isAnalyzing} className="m-0 min-w-0 border-0 p-0">
    <MealTypeSelector disabled={props.lockItemSet} selectedMealType={props.mealType} onSelect={props.onMealTypeChange} />
    {props.lockItemSet && <p className="mb-control-wide text-caption text-muted">The linked Diary date, meal, and food-item set stay unchanged. Add details or photo angles to improve the estimate.</p>}
    {!props.items.length && <PhotoCapturePrompt onChooseFromLibrary={() => newLibrary.current?.click()} onTakePhoto={() => newCamera.current?.click()} />}
    <div className="grid gap-control-wide">{props.items.map((item, index) => <CaptureFoodItemCard key={item.id} item={item} number={index + 1} preserveItem={props.lockItemSet} onAddFiles={props.onAddFiles} onDelete={props.onPhotoDelete} onNoteChange={props.onPhotoNoteChange} onUpdate={props.onItemChange} />)}</div>
    {props.items.length > 0 && !props.lockItemSet && <div className="my-control-wide grid gap-control rounded-sheet border border-dashed border-primary/40 p-control-wide"><strong className="text-detail">Add new food item</strong><div className="grid gap-control compact:grid-cols-2"><Button variant="secondary" onClick={() => newCamera.current?.click()}><Camera />Camera</Button><Button variant="secondary" onClick={() => newLibrary.current?.click()}><ImagePlus />Camera roll</Button></div></div>}
