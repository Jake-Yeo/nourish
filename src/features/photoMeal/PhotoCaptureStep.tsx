...
export function PhotoCaptureStep(props: PhotoCaptureStepProps) {
  ...
  const selectPhotos = () => photoInput.current?.click()
  return <div>
    <MealTypeSelector selectedMealType={props.mealType} onSelect={props.onMealTypeChange} />
    {props.photos.length ? <CapturedPhotoGrid photos={props.photos} onAddPhotos={selectPhotos} onDelete={props.onPhotoDelete} onNoteChange={props.onPhotoNoteChange} /> : <PhotoCapturePrompt onSelectPhotos={selectPhotos} />}
    <input ref={photoInput} hidden type="file" accept="image/*" capture="environment" multiple onChange={event => { props.onAddFiles(event.target.files); event.target.value = '' }} />
