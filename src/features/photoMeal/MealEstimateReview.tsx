...
export function MealEstimateReview({ actionLabel = 'Log estimated meal', estimate, mealType, onFieldChange, onLog, onNutrientChange, requireConfirmation, isLogging = false, onDelete, isDeleting = false }: Props) {
  ...
  return <div>
    ...
    {confirmingDelete && onDelete ? <div className="grid gap-control rounded-control bg-destructive-soft p-control-wide"><p className="text-caption text-destructive">Delete this analysis and its stored photos? Any Diary meal you already logged stays unchanged.</p><div className="grid grid-cols-2 gap-control"><Button variant="secondary" disabled={isDeleting} onClick={() => setConfirmingDelete(false)}>Keep analysis</Button><Button variant="destructive" className="rounded-control border border-destructive/30 bg-surface" loading={isDeleting} onClick={onDelete}><Trash2 />Delete permanently</Button></div></div> : confirming ? <div className="grid gap-control rounded-control bg-destructive-soft p-control-wide"><p className="text-caption text-destructive">This replaces only the linked Nourish photo entries. Other diary entries are unchanged.</p><div className="flex gap-control"><Button variant="secondary" disabled={isLogging} onClick={() => setConfirming(false)}>Cancel</Button><Button variant="destructive" loading={isLogging} onClick={() => onLog(mealType)}><Trash2 />Confirm replacement</Button></div></div> : requireConfirmation ? <Button fullWidth loading={isLogging} onClick={() => setConfirming(true)}><Check />{actionLabel}</Button> : <div className={onDelete ? 'grid grid-cols-2 gap-control' : ''}><Button fullWidth loading={isLogging} disabled={isDeleting} onClick={() => onLog(mealType)}><Check />{actionLabel}</Button>{onDelete && <Button variant="destructive" fullWidth className="rounded-control border border-destructive/30 bg-surface hover:bg-destructive-soft" disabled={isLogging} onClick={() => setConfirmingDelete(true)}><Trash2 />Delete analysis</Button>}</div>}
  </div>
}
