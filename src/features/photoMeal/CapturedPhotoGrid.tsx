...
type CapturedPhotoGridProps = { photos: CapturedPhoto[]; hideAddControls?: boolean; readOnly?: boolean; onChooseFromLibrary: () => void; onTakePhoto: () => void; onDelete: (photoId: string) => void; onNoteChange: (photoId: string, note: string) => void }

export function CapturedPhotoGrid({ hideAddControls, onChooseFromLibrary, onDelete, onNoteChange, onTakePhoto, photos, readOnly }: CapturedPhotoGridProps) {
  ...
  return <div className="grid gap-control-wide">
    {photos.map((photo, photoIndex) => <CapturedPhotoCard key={photo.id} photo={photo} photoNumber={photoIndex + 1} onDelete={onDelete} onNoteChange={onNoteChange} onView={(selected, number) => setViewedPhoto({ photo: selected, number })} readOnly={readOnly} />)}
