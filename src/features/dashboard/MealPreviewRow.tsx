...
export function MealPreviewRow({ entries, mealType, onQuickAdd }: { entries: DiaryEntry[]; mealType: MealType; onQuickAdd: (mealType: MealType) => void }) {
  const mealEntries = entries.filter(entry => entry.meal === mealType)
  const mealCalories = getTotalNutrients(mealEntries).calories
  return <div className="grid grid-cols-diary-row items-center gap-control-wide border-t border-border py-control-wide">
    <span className={mealIconVariants({ meal: mealType })}><Apple className="w-5" /></span>
