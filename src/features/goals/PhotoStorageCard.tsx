...
export function PhotoStorageCard() {
  ...
  return <Card variant="flat" className="grid gap-control-wide">
    ...
    {confirming ? <div className="grid gap-control rounded-control bg-destructive-soft p-control-wide"><p className="text-caption text-destructive">Delete all saved meal photos and notes? Nutrition diary entries and goals will remain.</p><div className="flex gap-control"><Button variant="secondary" onClick={() => setConfirming(false)}>Cancel</Button><Button variant="destructive" onClick={clear}><Trash2 />Clear photos</Button></div></div> : <Button variant="destructive" disabled={!usage?.photoCount} onClick={() => setConfirming(true)}><Trash2 />Clear photos</Button>}
  </Card>
}
