...
export function AnalysisJobDetail({ job, onDelete, onLog, onOpenDiary, onRefresh, onRerun }: Props) {
  ...
  return <div className="grid gap-content">
    ...
    {job.status === 'completed' && estimate && <>{job.error && <Card variant="flat" className="border-warning bg-warning-soft"><Typography className="text-warning">The latest re-run did not finish, so Nourish restored the previous analysis. {job.error}</Typography></Card>}{job.loggedAt ? <Card variant="soft"><Badge>Complete</Badge><Typography variant="body" className="mt-control-wide font-bold">Logged to your diary</Typography><Typography variant="muted" className="mt-control">This estimate remains available here as analysis history.</Typography><Button className="mt-content" onClick={() => onOpenDiary(job)}><BookOpen className="w-5" />Open in Diary</Button></Card> : <section><SectionHeader eyebrow="Ready to review" title="Check your estimate" /><MealEstimateReview estimate={estimate} isDeleting={action === 'delete'} isLogging={action === 'log'} mealType={job.source.mealType} onDelete={() => void remove()} onFieldChange={updateField} onNutrientChange={updateNutrient} onLog={() => void log()} /><Button fullWidth variant="secondary" className="mt-control-wide" disabled={action !== null} onClick={() => onRerun(job)}><RotateCcw className="w-5" />Edit photos &amp; notes, then re-run AI</Button></section>}</>}
  </div>
}
