...
type DiaryViewProps = { entries: DiaryEntry[]; focusMeal: MealType | null; selectedDateKey: string; onAdd: (meal: MealType) => void; onDelete: (id: string) => Promise<boolean>; onFocusMealConsumed: () => void; onMove: (ids: string[], date: string, meal: MealType) => Promise<boolean>; onSelect: (entry: DiaryEntry) => void }
...
export function DiaryView({ entries, focusMeal, selectedDateKey, onAdd, onDelete, onFocusMealConsumed, onMove, onSelect }: DiaryViewProps) {
  ...
  const deleteEntry = async (entryId: string) => {
    ...
    try {
      if (await onDelete(entryId)) setSelectedEntryIds(current => { const next = new Set(current); next.delete(entryId); return next })
    } finally { deletingEntryIdsRef.current.delete(entryId); setDeletingEntryIds(new Set(deletingEntryIdsRef.current)) }
  }
  ...
  return <>{selectedEntryIds.size > 0 && <div className="flex items-center justify-between rounded-control border border-primary bg-primary-soft p-control-wide"><span className="text-detail font-bold">{selectedEntryIds.size} item{selectedEntryIds.size === 1 ? '' : 's'} selected</span><Button size="compact" onClick={() => setIsMoveOpen(true)}>Move</Button></div>}{mealTypes.map(mealType => <MealDiaryCard deletingEntryIds={deletingEntryIds} entries={entries} id={`diary-meal-${mealType.toLowerCase()}`} key={mealType} mealType={mealType} selectedEntryIds={selectedEntryIds} onAdd={onAdd} onDelete={entryId => void deleteEntry(entryId)} onSelect={onSelect} onToggle={toggleEntry} />)}{isMoveOpen && <MoveDiaryModal count={selectedEntryIds.size} initialDate={selectedDateKey} onClose={() => setIsMoveOpen(false)} onMove={moveEntries} />}</>
}
