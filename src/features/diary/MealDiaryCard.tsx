...
type MealDiaryCardProps = { entries: DiaryEntry[]; id: string; mealType: MealType; selectedEntryIds: Set<string>; onAdd: (meal: MealType) => void; onDelete: (id: string) => void; onSelect: (entry: DiaryEntry) => void; onToggle: (entryId: string) => void }

export function MealDiaryCard({ entries, id, mealType, selectedEntryIds, onAdd, onDelete, onSelect, onToggle }: MealDiaryCardProps) {
  ...
  return <Card variant="flat" id={id} tabIndex={-1} aria-label={`${mealType} diary section`} className="scroll-mt-content focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-primary/20">
    ...
    {mealEntries.length === 0
      ...
      : mealEntries.map(diaryEntry => <DiaryEntryRow diaryEntry={diaryEntry} isSelected={selectedEntryIds.has(diaryEntry.id)} key={diaryEntry.id} onDelete={onDelete} onSelect={onSelect} onToggle={onToggle} />)}
  </Card>
}
