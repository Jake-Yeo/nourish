...
const maintenanceCalorieThreshold = 1700
...
export function CalorieTrendCard({ endingDateKey, nutritionData }: { endingDateKey: string; nutritionData: AppData }) {
  ...
  const assumedMaintenanceDays = dailyNutrition.filter(day => day.totalNutrients.calories <= maintenanceCalorieThreshold).length
  const adjustedCalories = dailyNutrition.map(day => day.totalNutrients.calories <= maintenanceCalorieThreshold ? nutritionData.goals.maintenanceCalories : day.totalNutrients.calories)
