...
export function AppHeader({ activeView, isSyncing, syncStatus, onGoalsOpen, onSync, onTodayOpen }: AppHeaderProps) {
  return <header className="mb-card flex items-start justify-between compact:items-center">
    ...
    <div className="flex items-center gap-control">
      ...
      <Button variant="secondary" size="compact" className="text-primary" onClick={onSync} disabled={isSyncing} aria-label={isSyncing ? `MyNetDiary sync ${syncStatus}` : 'Sync MyNetDiary'}><RefreshCw className={cn('w-4', isSyncing && 'animate-spin')} />{isSyncing ? syncStatus === 'queued' ? 'Queued' : 'Syncing' : 'Sync'}</Button>
