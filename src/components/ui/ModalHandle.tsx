...
export function ModalHandle({ disabled = false, onClose }: { disabled?: boolean; onClose: () => void }) {
  ...
  return <button type="button" disabled={disabled} className={cn('mx-auto mb-control block h-7 w-14 touch-none cursor-grab bg-transparent active:cursor-grabbing disabled:cursor-not-allowed disabled:opacity-50')} aria-label="Dismiss" style={{ transform: `translateY(${dragOffset}px)` }} onClick={onClose}
